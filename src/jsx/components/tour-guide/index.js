...
const TourGuide = (props) => {
  ...
  const sort = 5;
  let paggination = Array(Math.ceil(tourGuides.length / sort))
    .fill()
    .map((_, i) => i + 1);

  const activePag = useRef(0);
  const jobData = useRef(
    tourGuides.slice(activePag.current * sort, (activePag.current + 1) * sort)
  );

  const onClick = (i) => {
    activePag.current = i;
    jobData.current = tourGuides.slice(
                  ...
                  >
                          ...
                          {jobData.current.map((trans) => {
                            return (
                              <tr role="row" className="odd" key={trans.id}>
                                ...
                                <td>
                                  <div>
                                    <h5>{trans.tourGuideLanguageSpoken}</h5>
                                  </div>
                                </td>
                                {/* <td>
                                  <h5>{mentor.gender}</h5>
                    ...
                    <div className="d-sm-flex text-center justify-content-between align-items-center mt-3">
                      <div className="dataTables_info">
                        ...
                        {tourGuides.length > (activePag.current + 1) * sort
                          ? (activePag.current + 1) * sort
                          : tourGuides.length}{" "}
                        of {tourGuides.length} entries
                      </div>
                      <div
